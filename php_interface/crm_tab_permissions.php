 ...
 * 
 * 1. ID групп пользователей в Bitrix:
 *    - 1 = Администраторы
 *    - 5 = Модератор контента (часто используется для менеджеров)
 *    - Другие ID зависят от вашей настройки
 * 
 * 2. Типы действий:
 *    - READ   = Просмотр
 *    - WRITE  = Редактирование и добавление
 *    - DELETE = Удаление
 * 
 * 3. Логика работы с блоками:
 *    Если вкладка содержит несколько HL-блоков или инфоблоков,
 *    можно настроить отдельные права для каждого блока.
 *    Например:
 *    - Блок 1: Полный доступ для всех
 *    - Блок 2: Только чтение для обычных пользователей
 *    - Блок 3: Скрыт для всех, кроме администраторов
 * 
 * 4. Приоритет проверок:
 *    - Сначала проверяется доступ к вкладке целиком
 *    - Затем доступ к конкретному блоку (если настроено)
 *    - Затем доступ к полям (если настроено)
 * 
 * 5. Если конфигурация для вкладки не указана - доступ разрешен по умолчанию
 *    (с учетом базовых прав CRM)
 */