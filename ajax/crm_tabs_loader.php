<?php
/**
 * Универсальный AJAX загрузчик для вкладок CRM
 * /local/ajax/crm_tabs_loader.php
 */

use Bitrix\Main\Loader;
use Bitrix\Main\Application;

// Определяем лог-файл ДО подключения Bitrix
if (!defined('LOG_FILENAME')) {
    define('LOG_FILENAME', $_SERVER['DOCUMENT_ROOT'] . '/local/logs/crm_tabs.log');
}

error_reporting(E_ALL);
ini_set('display_errors', 0);
ini_set('log_errors', 1);

define('NO_KEEP_STATISTIC', true);
define('NOT_CHECK_PERMISSIONS', false);
define('NO_AGENT_CHECK', true);

// Подключение Битрикс
require_once $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/include/prolog_before.php';

/**
 * Возврат ошибки
 */
function returnAjaxError($message, $code = 500, $details = []) {
    AddMessage2Log("CRM Tab AJAX Error: {$message} | " . json_encode($details, JSON_UNESCAPED_UNICODE), 'crm_tabs');
    
    http_response_code($code);
    header('Content-Type: application/json; charset=utf-8');
    
    echo json_encode([
        'success' => false,
        'error' => $message,
    ], JSON_UNESCAPED_UNICODE);
    die();
}

// Обработчик фатальных ошибок
register_shutdown_function(function() {
    $error = error_get_last();
    if ($error && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        AddMessage2Log("CRM Tab Fatal: {$error['message']} in {$error['file']}:{$error['line']}", 'crm_tabs');
        
        if (ob_get_length()) {
            ob_clean();
        }
        
        http_response_code(500);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['success' => false, 'error' => 'Критическая ошибка сервера'], JSON_UNESCAPED_UNICODE);
    }
});

try {
    global $APPLICATION, $USER;
    
    AddMessage2Log("=== CRM Tab AJAX ===", 'crm_tabs');
    
    // Проверка авторизации
    if (!$USER->IsAuthorized()) {
        returnAjaxError('Необходима авторизация', 403);
    }
    
    // Подключение модулей
    if (!Loader::includeModule('crm')) {
        returnAjaxError('Модуль CRM не установлен', 500);
    }
    
    if (!Loader::includeModule('highloadblock')) {
        returnAjaxError('Модуль Highloadblock не установлен', 500);
    }
    
    $request = Application::getInstance()->getContext()->getRequest();
    
    // === Парсинг параметров ===
    $componentName = null;
    $companyId = 0;
    $tabCode = '';
    
    // 1. GET/POST параметры
    $componentName = $request->get('component') ?: $request->getPost('component');
    $companyId = intval($request->get('companyId') ?: $request->getPost('companyId'));
    $companyId = $companyId ?: intval($request->get('COMPANY_ID') ?: $request->getPost('COMPANY_ID'));
    $tabCode = $request->get('tabCode') ?: $request->getPost('tabCode');
    
    // 2. Bitrix loader формат PARAMS
    $loaderParams = $request->get('PARAMS') ?: $request->getPost('PARAMS');
    if (is_string($loaderParams)) {
        $loaderParams = json_decode($loaderParams, true);
    }
    
    if (is_array($loaderParams)) {
        // Подписанные параметры
        if (!empty($loaderParams['signedParameters']) && !empty($loaderParams['component'])) {
            try {
                $signedParams = \Bitrix\Main\Component\ParameterSigner::unsignParameters(
                    $loaderParams['signedParameters'],
                    $loaderParams['component']
                );
                if (!$companyId && !empty($signedParams['COMPANY_ID'])) {
                    $companyId = intval($signedParams['COMPANY_ID']);
                }
                if (!$tabCode && !empty($signedParams['TAB_CODE'])) {
                    $tabCode = $signedParams['TAB_CODE'];
                }
                if (!$componentName && !empty($loaderParams['component'])) {
                    $componentName = str_replace('custom:', '', $loaderParams['component']);
                }
            } catch (\Exception $e) {
                AddMessage2Log("Signature error: " . $e->getMessage(), 'crm_tabs');
            }
        }
        
        // Простые параметры
        if (!$companyId && !empty($loaderParams['params']['COMPANY_ID'])) {
            $companyId = intval($loaderParams['params']['COMPANY_ID']);
        }
        if (!$tabCode && !empty($loaderParams['params']['TAB_CODE'])) {
            $tabCode = $loaderParams['params']['TAB_CODE'];
        }
    }
    
    // 3. JSON body
    $rawInput = file_get_contents('php://input');
    if (!empty($rawInput) && $rawInput[0] === '{') {
        $jsonData = json_decode($rawInput, true);
        if (is_array($jsonData)) {
            if (!$componentName && !empty($jsonData['component'])) {
                $componentName = $jsonData['component'];
            }
            if (!$companyId && !empty($jsonData['companyId'])) {
                $companyId = intval($jsonData['companyId']);
            }
            if (!$companyId && !empty($jsonData['PARAMS']['params']['COMPANY_ID'])) {
                $companyId = intval($jsonData['PARAMS']['params']['COMPANY_ID']);
            }
        }
    }
    
    // 4. Определение компонента из tabCode
    if (!$componentName && $tabCode) {
        $tabToComponent = [
            'tab_outlets' => 'crm.company.tab.outlets',
            'tab_contracts' => 'crm.company.tab.contracts',
            // Добавляйте другие вкладки здесь
        ];
        $componentName = $tabToComponent[$tabCode] ?? null;
    }
    
    AddMessage2Log("Parsed: component={$componentName}, companyId={$companyId}, tabCode={$tabCode}", 'crm_tabs');
    
    // Валидация
    if (!$componentName) {
        returnAjaxError('Не указан компонент', 400);
    }
    
    if (!$companyId) {
        returnAjaxError('Не указан ID компании', 400);
    }
    
    // Проверка прав CRM
    $crmPerms = new CCrmPerms($USER->GetID());
    $permType = $crmPerms->GetPermType('COMPANY', 'READ');
    if ($permType === BX_CRM_PERM_NONE && !$USER->IsAdmin()) {
        returnAjaxError('Недостаточно прав', 403);
    }
    
    // Проверка компонента
    $componentPath = $_SERVER['DOCUMENT_ROOT'] . '/local/components/custom/' . $componentName;
    if (!file_exists($componentPath . '/class.php')) {
        returnAjaxError('Компонент не найден: ' . $componentName, 404);
    }
    
    AddMessage2Log("Including component: {$componentName}", 'crm_tabs');
    
    // Выполнение компонента
    ob_start();
    
    $APPLICATION->IncludeComponent(
        'custom:' . $componentName,
        '.default',
        [
            'COMPANY_ID' => $companyId,
            'TAB_CODE' => $tabCode ?: str_replace('crm.company.', '', $componentName),
        ],
        false,
        ['HIDE_ICONS' => 'Y']
    );
    
    $content = ob_get_clean();
    
    AddMessage2Log("Output length: " . strlen($content), 'crm_tabs');
    
    // Определяем тип ответа
    $trimmedContent = ltrim($content);
    if ($trimmedContent && $trimmedContent[0] === '{') {
        header('Content-Type: application/json; charset=utf-8');
    } else {
        header('Content-Type: text/html; charset=utf-8');
    }
    
    echo $content;
    
} catch (\Exception $e) {
    AddMessage2Log("Exception: " . $e->getMessage(), 'crm_tabs');
    returnAjaxError('Ошибка: ' . $e->getMessage(), 500);
}
